    ...
    $(document).ready(function() {
        ...
        function fetchProducts() {
            $.ajax({
                ...
                error: function() {
                    $('#main-loader').html('<p class="text-danger">Failed to load products from server.</p>');
                }
            });
        }
        ...
        function renderProducts(data) {
            ...
            if (!data.products || data.products.length === 0) {
                html = '<div class="col-12 text-center py-5"><p class="alert alert-info">No products found.</p></div>';
            } else {
        ...
        $(document).on('click', '.quick-view', function(e) {
            ...
            $modalContent.html(`<div class="modal-body text-center py-5"><div class="spinner-border text-primary"></div></div>`);
            ...
            $.get(`/product-details/${id}`, function(res) {
                if (res.status === 'success') {
                    ...
                    let wishlist = getStoredItems('user_wishlist');
                    let cart = getStoredItems('user_cart');
                    if (wishlist.includes(id.toString())) $modalContent.find('.wishlist-toggle-btn i').removeClass('fa-regular').addClass('fa-solid text-danger');
                    if (cart.some(item => item.id == id.toString())) $modalContent.find('.add-to-cart-btn').html('<i class="fa-solid fa-check me-2"></i> In Cart').addClass('btn-success disabled').attr('disabled', true);
                }
            });
        });
        ...
        $(document).on('click', '.wishlist-toggle-btn', function(e) {
            ...
            if (isAdding) wishlist.push(id); else wishlist = wishlist.filter(itemId => itemId !== id);
            setStoredItems('user_wishlist', wishlist);
            syncWishlistUI(id, isAdding);
            updateNavCount();
            if ($('#wishlistDrawer').hasClass('show')) loadWishlistDrawer();
            Toast.fire({ icon: 'success', title: isAdding ? "Added to wishlist" : "Removed from wishlist" });
        });

        function syncWishlistUI(id, isAdded) {
            $(`.wishlist-toggle-btn[data-id="${id}"] i`).each(function() {
                if (isAdded) $(this).removeClass('fa-regular text-dark').addClass('fa-solid text-danger');
                else $(this).removeClass('fa-solid text-danger').addClass('fa-regular text-dark');
            });
        }
        ...
        $(document).on('click', '.add-to-cart-btn', function(e) {
            ...
            if (!cart.some(item => item.id === id)) {
                ...
                $(`.add-to-cart-btn[data-id="${id}"]`).each(function() {
                    $(this).html('<i class="fa-solid fa-check me-1"></i> In Cart').removeClass('btn-soft-primary').addClass('btn-success disabled').attr('disabled', true);
                });
                updateNavCount();
        ...
        $(document).on('click', '.remove-cart-item', function(e) {
            ...
            setStoredItems('user_cart', cart);
            $(`.add-to-cart-btn[data-id="${id}"]`).html('<i class="fa-solid fa-plus me-1"></i> Add to Cart').removeClass('btn-success disabled').addClass('btn-soft-primary').attr('disabled', false);
            updateNavCount(); loadCartDrawer();
        });
        ...
        window.loadWishlistDrawer = function() {
            ...
            $.get("{{ route('wishlist.index') }}", { ids: ids }, function(res) { $('#wishlist-drawer-content').html(res.html); });
        };

        window.loadCartDrawer = function() {
            let items = getStoredItems('user_cart');
            $.get("{{ route('cart.index') }}", { items: items }, function(res) { $('#cart-drawer-content').html(res.html); });
        };

        $(document).on('click', '.trigger-wishlist', function(e) { e.preventDefault(); new bootstrap.Offcanvas('#wishlistDrawer').show(); loadWishlistDrawer(); });
        $(document).on('click', '.trigger-cart', function(e) { e.preventDefault(); new bootstrap.Offcanvas('#cartDrawer').show(); loadCartDrawer(); });

        // Checkout Process Logic
        $(document).on('click', '.proceed-checkout', function(e) {
            e.preventDefault();
            let cart = getStoredItems('user_cart');
            if (cart.length === 0) { Toast.fire({ icon: 'warning', title: 'Your cart is empty!' }); return; }

            let fullCartDetails = [];
            $('.cart-item-card').each(function() {
                fullCartDetails.push({
                    name: $(this).find('h6').text().trim(),
                    price: parseFloat($(this).find('.text-primary').text().replace(/[^\d.]/g, '')),
            ...
            $.ajax({
                ...
                method: "POST",
                data: { _token: "{{ csrf_token() }}", items: fullCartDetails },
                beforeSend: function() { $('.proceed-checkout').html('<span class="spinner-border spinner-border-sm"></span> Loading...'); },
                success: function(res) {
                    if (res.status === 'success') {
                        ...
                        $('#chk-subtotal').text('৳' + new Intl.NumberFormat().format(subtotal));
                        $('#chk-total').text('৳' + new Intl.NumberFormat().format(total));

                        let deliveryText = (deliveryCharge === 0) ? '<span class="text-success fw-bold">FREE</span>' : '৳' + deliveryCharge;
                        $('#chk-delivery-charge').html(deliveryText);

                        bootstrap.Offcanvas.getInstance('#cartDrawer')?.hide();
                        setTimeout(() => { new bootstrap.Offcanvas('#checkoutDrawer').show(); }, 400);
                    }
                },
                complete: function() { $('.proceed-checkout').html('Checkout <i class="fa-solid fa-arrow-right ms-2"></i>'); }
            });
        });
        ...
        $(document).on('submit', '#checkoutForm', function(e) {
            ...
            updateNavCount();
            $('.add-to-cart-btn').html('<i class="fa-solid fa-plus me-1"></i> Add to Cart').removeClass('btn-success disabled').addClass('btn-soft-primary').attr('disabled', false);
            bootstrap.Offcanvas.getInstance('#checkoutDrawer')?.hide();

            Swal.fire({ icon: 'success', title: 'Order Successful!', text: 'Order ID: ' + orderData.order_id, timer: 3000, showConfirmButton: false });
            this.reset();
        });
        ...
        function renderOrderHistory() {
            ...
            let html = '';
            if (orders.length === 0) {
                html = `<div class="text-center py-5"><i class="fa-solid fa-box-open fa-3x text-light mb-3"></i><p class="text-muted">No orders found yet!</p></div>`;
            } else {
        ...
        $(document).on('click', '.trigger-orders', function(e) {
            ...
            $.get("{{ route('order.index') }}", function(res) {
                ...
                new bootstrap.Offcanvas('#orderHistoryDrawer').show(); // Ensure this ID exists in your layout
                renderOrderHistory();
            });
        });
